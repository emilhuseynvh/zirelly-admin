...
const FormSchema = z.object({
  name: z.string().min(5, {
    message: "role name must be at least 5 characters."
  }),
  email: z
    .string()
    .min(1, { message: "Email mütləq daxil edilməlidir." }) 
...
export default function UpdateUserForm() {
    ...
    useEffect(()=>{
       fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/${id}/edit`)
        .then((response) => response.json()) 
        .then((result) => {
         if (result && result.data) {
          setEditUser(result.data.user)
  ...
  await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/${id}/update`, {
  method: "PATCH",
  headers: {
